...
## Basic Challenge
I’ll start by opening my `Fastfile`. As we saw in the previous episode, adding a description for an action simply requires invoking the `desc` keyword following by a string describing the lane. We do this immediately before the lane's declaration:
```ruby
...
```
As expected, fastlane cruises on over to  `sync_device_info` first, and once finished, it returns to our original lane and completes the remainder of it. Let's take a look at the lane documentation in the auto-generated `fastlane/README.md` file:
<!-- Show code + markdown preview -->
Nice! 
## Advanced Challenge
For our advanced challenge, I start by opening the documentation for `register_devices`, and then I create a new `Devicefile` inside the project `fastlane` folder.
I grab the basic content of the downloaded tab-separated iOS device configuration file and pop that into my new file. I get the device names and UDID's from my existing inline device list and save my changes.
```
...
```
One thing here that can easily trip you up is that the path required is relative to the project root, while you might expect it to be relative to your ``fastlane``` directory. If you ever get this wrong, `fastlane's` error messages will give you helpful feedback.
I’ll try out our updated lane in the CLI:
```ruby
fastlane sync_device_info
```
Nice. The new device list works as expected. Over time, this kind of separation really helps avoid massive `Fastfile` syndrome and keeps everything lean and clean.
# Conclusion
